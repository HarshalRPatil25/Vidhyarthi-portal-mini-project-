...
if (isset($_POST['submit'])) {
    ...
    $sql = "UPDATE `$table` Set `$curDate`='0' where Roll_no in ($absent)";
    $sql1 = "UPDATE `$table` Set `$curDate`='1' where Roll_no not in ($absent)";


    if ($con->query($insertcolumn) === TRUE) {
        if ($con->query($sql) === TRUE) {
            echo "<h3>Your Mention Students For Date $curDate Are Now Mark As Absent!!! </h3>";
        } else {
...
<header>
    ...
    <div class="login-form">
        <form method="post">
            <h1><b>For Max Present</b></h1>
            <div class="content">
                <div class="input-details">
                    
                    <input name="absent" type="text" placeholder="Enter the Absent Roll Numbers e.g. 1,3,4"></input>
                </div>
